"""Review bundle structure tests for fft_transform."""
...
def test_fft_transform_review_bundle_structure() -> None:
    bundle = _load_bundle(BUNDLE_DIR / "fft_transform.review_bundle.json")
...
def test_fft_transform_review_bundle_covers_expected_rows() -> None:
    bundle = _load_bundle(BUNDLE_DIR / "fft_transform.review_bundle.json")
    assert {row["atom_key"] for row in bundle["rows"]} == {
        "sciona.atoms.signal.spectral.fft_transform.apply_spectral_window",
        "sciona.atoms.signal.spectral.fft_transform.optimize_fft_length",
        "sciona.atoms.signal.spectral.fft_transform.compute_forward_rfft",
    }
